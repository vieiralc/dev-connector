...
router.post(
  '/',
  [auth_middleware, check('text', TEXT_REQUIRED).not().isEmpty()],
  async (req, res) => {
...
router.put('/like/:id', auth_middleware, async (req, res) => {
  try {
    ...
    const hasAuthenticatedUserLikedPost =
      post.likes.filter((like) => like.user.toString() === authenticatedUserId)
        .length > 0;

    if (hasAuthenticatedUserLikedPost) {
      return res
        .status(STATUS_400)
        .json({ errors: [{ msg: USER_ALREADY_LIKED_POST }] });
    }
...
router.delete('/comment/:id/:comment_id', auth_middleware, async (req, res) => {
  try {
    ...
    const commentsIdsArray = post.comments.map((comment) =>
